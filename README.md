...
コード内でBot名を参照している箇所は `src/config.js` の `BOT_NAME` 1箇所にまとめてあるので、
...
```
ganon-chan-bot/
├── deploy-commands.js       # 全機能のスラッシュコマンドを自動収集して登録
├── package.json
├── .env.example
└── src/
    ├── index.js             # エントリーポイント。interactionをcustomId/コマンド名で振り分けるだけ
    ├── config.js            # BOT_NAMEなど、Bot全体に関わる設定値
    ├── utils/
    │   └── draftStore.js    # 機能横断で使う、ユーザーごとの一時入力状態ストア
    └── features/
        ├── index.js         # features配下のフォルダを自動スキャンして読み込むローダー
        └── smashRecruit/    # 「募集機能」一式（1機能1フォルダ）
            ├── index.js         # この機能のcommands/componentsをまとめて返す窓口
            ├── constants.js     # customIdや選択肢ラベルなどの定数
            ├── command.js       # /smash-recruit の定義と初期応答
            ├── components.js    # セレクトメニュー・ボタン・モーダルの処理
            └── view.js          # メッセージ本文・コンポーネントの組み立てロジック
```
...
- **`src/features/index.js` はフォルダを自動スキャンするだけ**：新機能を追加してもこのファイルを編集する必要はない。
- **customIdは `機能名:アクション名` で統一**（例: `smash-recruit:submit`）：機能が増えてもcustomIdが衝突しない。
- **一時状態は `draftStore.js` に集約**：`getDraft(featureKey, userId)` のように機能名でネームスペースを分けるので、
  複数機能が同時に「選択中の入力」を持っても混ざらない。

## 新しい機能を追加する手順

1. `src/features/<新機能名>/` フォルダを作る
2. その中に、既存の `smashRecruit/` を参考に `constants.js` / `command.js` / `components.js` / `view.js` を作る
   （小さい機能ならファイルを分けず1〜2ファイルにまとめても構わない）
3. `index.js` で `export default { commands: [...], components: [...] }` を返す
4. `npm run deploy-commands` を実行すればコマンドが自動的に登録される

`src/index.js` や `deploy-commands.js` を書き換える必要はない。
...
「視聴中：ガノンちゃん稼働中」と表示される（`src/index.js` 内で `BOT_NAME` を使って設定している）。
...
- 機能が増えて複数人が同時にBotを使うようになった場合、`draftStore.js` のMapはプロセスをまたがないため、
