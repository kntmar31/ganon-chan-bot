...
Node.js は 20.9.0 以上が必要（画像の合成に使う `sharp` の要件。`package.json` の `engines` に指定している）。
...
- アイコンは、Discord 全体のアイコンを使います（サーバーごとのアイコンは使いません）。取得できなかった人（退会したユーザーや、7秒以内に応答が届かなかった場合を含む）は、灰色の丸で表示します
