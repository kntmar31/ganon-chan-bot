...
// 画像のピクセル数を多く(標準の2倍)して、画面の解像度が高いスマホでもボヤけないようにしている。
// 画像は、PC では最大 550px 幅、スマホでは画面の幅に合わせて縮小されて表示される。
// 縮小後のアイコンの大きさは、アイコンと画像の幅の比率で決まる(PC で約 62px、スマホで約 37px)。
// 1列ぶんの画像の幅は 8 * AVATAR_SIZE + 7 * AVATAR_GAP = 1704px で、アイコンと間隔の比率は 8:1
